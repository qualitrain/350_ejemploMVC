...
public class GestorBD {
	    ...
	    for (Throwable e : ex) {
	        if (e instanceof SQLException) {
	            ...
	            if (ignorarSQLException(((SQLException)e).getSQLState()) == false) {
	                ...
	                System.err.println("Código de error-->" + ((SQLException)e).getErrorCode()+"<--");
	...
	public static void mostrarCaracterísticasBD()throws SQLException{
		if (GestorBD.conexionBD==null){
			System.err.println("No hay conexión abierta");
			return;
		}
		try {
	...
	public static boolean ignorarSQLException(String sqlState) {
		    if (sqlState == null) {
		      System.out.println("El estado SQL no está definido!");
		      return false;
		    }
	...
	public static GrupoPersonas recuperarPersonasTodas()throws SQLException {
		...
		Statement stmt = null;
		try {
			if (GestorBD.conexionBD == null){ //No hay conexion de BD aún
...
//	        callStmt = conexionBD.prepareCall("{call ejemplos_jdbc.CUENTA_ARTI(?)}"); // quite el comentario para probar que sucede 
	        ...
	        /*
	         * Las líneas comentadas exploran los parámetros recibidos de la base de datos
	         * Es útil, porque cuando no se encuentra el procedimiento almacenado
	         * se lanza una excepción (en Mysql) con un error que dice
	         * "No output parameters returned by procedure" 
	         * o alguno que indica que el parámetro no es de tipo OUT:
	         * Parameter number 1 is not an OUT parameter, lo cual es muy críptico pq no indica que
	        ...
	        */ 
	        ...
	        callStmt.registerOutParameter(1, Types.INTEGER); //Registra el tipo del parámetro OUT 1
...
//					pStmt.setFloat(3, 36.0f);
		...
		try {
			...
			try {
				while(listaVendedores.getLLaves().hasNext()){
					...
					try{ // Recuperar las ventas del vendedor de la iteración
	...
	public static int insertarVentaNoTransaccional (Venta nuevaVenta)throws SQLException{
		...
		// No observa comportamiento transaccional: Permite actualizar la mitad o parte de una transacción
